...
@Suite
struct ProTools_SessionText_TracksOnly {
    @Test
    func sessionText_TracksOnly() throws {
        // load file
        ...
        #expect(
            try track1clip1.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 15, f: 06), at: .fps24))
        )
        #expect(
            try track1clip1.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 01, s: 05, f: 13), at: .fps24))
        )
        #expect(
            try track1clip1.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 50, f: 07), at: .fps24))
        )
        ...
        #expect(
            try track1clip2.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 01, s: 05, f: 13), at: .fps24))
        )
        #expect(
            try track1clip2.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 01, s: 57, f: 23), at: .fps24))
        )
        #expect(
            try track1clip2.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 52, f: 09), at: .fps24))
        )

...
@Suite
struct ProTools_SessionText_OneOfEverything {
    @Test
    func sessionText_OneOfEverything() throws {
        // load file
        ...
        #expect(
            try track1clip1.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 00, f: 00), at: .fps23_976))
        )
        #expect(
            try track1clip1.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 05, f: 00), at: .fps23_976))
        )
        #expect(
            try track1clip1.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 05, f: 00), at: .fps23_976))
        )
        ...
        #expect(
            try track2clip1.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 06, f: 15), at: .fps23_976))
        )
        #expect(
            try track2clip1.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 10, f: 03), at: .fps23_976))
        )
        #expect(
            try track2clip1.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 03, f: 12), at: .fps23_976))
        )
        ...
        #expect(
            try track3clip1.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 12, f: 18), at: .fps23_976))
        )
        #expect(
            try track3clip1.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 17, f: 08), at: .fps23_976))
        )
        #expect(
            try track3clip1.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 04, f: 14), at: .fps23_976))
        )
        ...
        #expect(
            try track3clip2.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 18, f: 17), at: .fps23_976))
        )
        #expect(
            try track3clip2.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 21, f: 19), at: .fps23_976))
        )
        #expect(
            try track3clip2.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 03, f: 02), at: .fps23_976))
        )
        ...
        #expect(
            try track3clip3.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 12, f: 18), at: .fps23_976))
        )
        #expect(
            try track3clip3.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 17, f: 08), at: .fps23_976))
        )
        #expect(
            try track3clip3.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 04, f: 14), at: .fps23_976))
        )
        ...
        #expect(
            try track3clip4.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 18, f: 17), at: .fps23_976))
        )
        #expect(
            try track3clip4.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 21, f: 19), at: .fps23_976))
        )
        #expect(
            try track3clip4.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 03, f: 02), at: .fps23_976))
        )
        ...
        #expect(
            try track5clip1.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 05, f: 14), at: .fps23_976))
        )
        #expect(
            try track5clip1.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 11, f: 10), at: .fps23_976))
        )
        #expect(
            try track5clip1.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 05, f: 20), at: .fps23_976))
        )
        ...
        #expect(
            try track5clip2.startTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 05, f: 14), at: .fps23_976))
        )
        #expect(
            try track5clip2.endTime
                == .timecode(ProTools.formTimecode(.init(h: 01, m: 00, s: 11, f: 10), at: .fps23_976))
        )
        #expect(
            try track5clip2.duration
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 00, s: 05, f: 20), at: .fps23_976))
        )
        ...
        #expect(
            try marker1.location
                == .timecode(ProTools.formTimecode(.init(h: 00, m: 59, s: 58, f: 00), at: .fps23_976))
        )

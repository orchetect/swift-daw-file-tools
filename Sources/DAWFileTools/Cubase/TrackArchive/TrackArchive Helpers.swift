...
extension Cubase.TrackArchive {
    ...
    func calculateStartTimecode(ofRealTimeValue: TimeInterval?) -> Timecode? {
    ...
    /// Requires main.frameRate to not be nil.
    /// Real Time measured in seconds.
    func calculateLengthTimecode(ofRealTimeValue: TimeInterval?) -> Timecode? {
        guard let ofRealTimeValue else { return nil }
    ...
    /// Internal use.
    /// Requires main.frameRate to not be nil.
    func calculateStartTimecode(ofMusicalTimeValue: Double) -> Timecode? {
    ...
    /// Internal use.
    /// Requires main.frameRate to not be nil.
    func calculateLengthTimecode(ofMusicalTimeValue: Double) -> Timecode? {
        let realTimeSeconds = calculateMusicalTimeToRealTime(ofMusicalTimeValue: ofMusicalTimeValue)
    ...
    func calculateMusicalTimeToRealTime(
        ...
        for element in self {
            ...
            case let marker as Cubase.TrackArchive.Marker:
                outputString += "\(marker.name)".tabbed
                    + marker.startTimecode.stringValue(format: Cubase.timecodeStringFormat).newLined
            ...
            case let marker as Cubase.TrackArchive.CycleMarker:
                outputString += "\(marker.name)".tabbed
                    ...
                    + marker.lengthTimecode.stringValue(format: Cubase.timecodeStringFormat).newLined

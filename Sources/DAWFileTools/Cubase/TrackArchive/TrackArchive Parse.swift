...
extension Cubase.TrackArchive {
    static func parse(fileContent xml: XMLDocument) throws -> (
        trackArchive: Self,
    ...
    static func parse(fileContent xmlRoot: XMLElement) -> (
        trackArchive: Self,
        messages: [ParseMessage]
    ) {
        ...
        for event in events {
            ...
            case "MMarkerEvent", "MRangeMarkerEvent": // all marker event types
                ...
                case "MMarkerEvent": // single marker
                    guard let tcStart else { continue }
                    newMarker = AnyMarker(
                        Marker(
                            name: name ?? "",
                ...
                if let newMarker { newTrack.events.append(newMarker) }

...
extension ProTools.SessionInfo {
    struct ParsedTracks {
        ...
        struct TrackComponents {
            var name: String
            var comments: String
            var userDelay: String
            var state: String
            var plugins: String
            var clips: [ClipComponents] = []
        ...
        static func tracksLines(
            ...
            var hopper: [String] = []
            for line in lines {
                if line.hasPrefix(caseInsensitive: "TRACK NAME:") {
                    if !hopper.isEmpty { tracksLines.append(hopper) }
                    hopper.removeAll()
                }
                let lineToAdd = line.trimmingCharacters(in: .newlines)
                if !line.isEmpty { hopper.append(lineToAdd) }
            }
            if !hopper.isEmpty { tracksLines.append(hopper) }
            ...
            let clips: [TrackComponents.ClipComponents] = clipList
                ...
                .reduce(into: []) { base, clip in
                    ...
                    guard columns.count == 7 else {
                        let clipDetail = columns.map(\.quoted).joined(separator: ", ")
                        addParseMessage(.error(
    ...
    struct ParsedMarkers {
        ...
        static func markersComponents(
            ...
            let linesComponents: [MarkerComponents] = rawMarkers.reduce(into: []) { base, rawMarker in
                ...
                let commentNewLinePrefix: String = switch sectionVersion {
                case .legacy:
                    "    \t             \t                  \t           \t                                 \t"
                case .pt2023_12:
                    "    \t             \t                  \t           \t                                 \t                                 \t                                 \t"
                ...
                if let commentsSuffix {
                    ...
                    let dropLastCount: Int = switch sectionVersion {
                    case .legacy: 1
                    case .pt2023_12: 3
                    }
